 ...
 * - Writes logs by severity to `logs/<date>/<severity>.log`
 * - Mirrors every line to the console so process managers pick it up too
 * - Enriches lines with the request's correlation ID, IP and identity when
 *   called from inside a request (see `correlationId.middleware.ts`)
 */
export class LogHelper {
  ...
  private static getSeverityFilePath(
    dateDirPath: string,
  ...
  /**
   * Logs errors/warnings to a file.
   * @param route API route or function name
